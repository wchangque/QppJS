...
- [x] **ES2022 Class Private Fields（`#x`）**（2026-06-02）：
  ...
  - **Parser**：`parse_class_body` 在 key 解析时检测 PrivateName token，标记 `is_private=true`；`led(Dot)` 检测下一 token 为 PrivateName → 产生 `PrivateMemberExpression`；`led(KwIn)` 左侧为 PrivateName → 产生 `PrivateInExpression`；赋值目标（`=`/`+=` 等）LHS 为 PrivateMemberExpression 时，合成 `MemberAssignmentExpression` with `Identifier{#name}` 属性名；`++`/`--` update expr LHS 为 PrivateMemberExpression 时同样转换。
  - **Interpreter**：`eval_class_common` 为每个私有字段名调用 `SymbolTable::NewSymbol` 分配唯一 symbol id，建立 `name → symbol_value` 映射，注入 ctor 和所有方法函数的 `private_fields_` 成员；`eval_expr` 新增 PrivateMemberExpression handler（通过 `private_fields_` 查 symbol id，调用 `get_property_by_symbol`；kFunction 对象走 `"__pfsym_<id>__"` 字符串属性后备）和 PrivateInExpression handler（`has_property_by_symbol`）；`eval_member_assign` 检测 `#name` Identifier 属性走 symbol `set_property_by_symbol` 路径（含 `+=`/`-=` 等复合赋值先读后写）；`eval_update_expr` PrivateMemberExpression 路径（read-modify-write via `private_fields_`）；`init_instance_fields` 私有字段使用 symbol key（`set_property_by_symbol`）。
  - **VM/Compiler**：`compiler.h` 新增 `SymbolTable`（`name → uint64_t id` 映射）和 `private_fields_stack_`（类嵌套栈）；`hoist_vars_scan_expr` 扩展 PrivateMemberExpression / PrivateInExpression 两个新节点遍历；`compile_expr` 新增 PrivateMemberExpression（`kLoadString(Value::symbol(id)) + compile(obj) + kGetElem`）和 PrivateInExpression（`kLoadString(Value::symbol(id)) + compile(obj) + kIn`）；`compile_member_assign` 检测 `#name` 私有字段走 symbol `kSetElem` 路径（含复合赋值 read-modify-write：`kDup + kLoadString(sym) + kGetElem + op + kSwap + kLoadString(sym) + kSetElem`）；`compile_update_expr` PrivateMemberExpression 内联 read-modify-write 序列；`compile_field_initializer` 私有字段 emit `kLoadString(symbol) + kSetElem`（覆盖原 `kSetProp`）；`compile_class_common` 扫描私有字段到 `private_fields_stack_`，static 私有字段在 kMakeClass 后 emit `kDup + kLoadString(sym) + kSetElem`；`vm.cpp` kGetElem / kSetElem 新增 kFunction + symbol key 分支（通过 `"__pfsym_<id>__"` 字符串代理存储于 `own_properties_`）。
  - **interpreter.h**：去除未用 `private_fields_stack_` 字段（Interpreter 侧通过 JSFunction `private_fields_` 传递）；`js_function.h` 新增 `private_fields_` 成员（`std::unordered_map<std::string, Value>`，name→symbol value 映射）。
  - **测试**：新增 `tests/unit/private_fields_test.cpp`（PF-01～PF-10 × Interp+VM = 20 个测试，覆盖基础读写、私有方法、静态私有字段、`#x in obj` brand check、私有字段 ++/--、子类私有字段隔离、多实例独立、复合赋值、访问不含私有字段的对象应报 TypeError）；`tests/CMakeLists.txt` 注册。
  - **测试结果**：4590/4590 通过（coverage），0 LSan 泄漏（预期）。
...
- [x] **计算属性键 T262-P2 Review 必修修复 M1/M2/P1**（2026-05-26）：
  ...
  - **M2：VM rest 排除计算键修复**：compiler.cpp `compile_bind_pattern` 中，当 `has_rest` 时为计算键分配 `$__qppjs_ckey_N__` 临时变量保存运行时键值，rest 阶段通过 `kGetVar` 追加到 `kCopyDataProperties` 的排除列表（总排除数 = 静态键数 + 计算键数）；vm.cpp `kCopyDataProperties` 排除键的处理从 `k.sv()`（仅适用于 string）改为 `to_string_val(k)`（正确处理数字/布尔/字符串等任意运行时键值），Symbol 键跳过（永不出现在 `own_enumerable_string_keys`）。
  - **P1：kDefineComputedGetter/kDefineComputedSetter 消除重复 to_string_val**：各 handler 新增 `std::string str_key = key.is_symbol() ? std::string{} : to_string_val(key)`，在 fn.name 设置和 define_property 两处统一引用 str_key，消除两次调用。
  - **新增测试**：6 个（CP-49/CP-50/CP-51 × Interp+VM，验证 M1 getter 调用、M1 setter 调用、M2 rest 正确排除）。
  - **测试结果**：4053/4053 通过（coverage），0 LSan 泄漏。

- [x] **计算属性键边界补测 [T262-P2 Testing]**（2026-05-26）：
  - **新增测试**：36 个（CP-31～CP-48 × Interp+VM），`tests/unit/computed_property_test.cpp` 总计 96 个测试。
  - **覆盖点**：解构默认值三场景（键存在/缺失/null）；嵌套解构 `{[key]:{b,c}}`；嵌套解构内层默认值 `{[key]:{b=10}={}}`；同名双计算键后写覆盖；computed+static/static+computed 同名后写覆盖；falsy 计算键（`false`→"false"、`0`→"0"、`""`→""）；`Object.getOwnPropertyDescriptor` 数据属性（value/writable/enumerable/configurable）和访问器属性（get/set 存在，value/writable 为 undefined）；计算方法 this 绑定；计算方法通过 this 连续修改属性（计数验证）；多 Symbol 键相互独立；`Object.getOwnPropertySymbols` 未实现记录（typeof → "undefined"）。
  - **发现 Bug（未在本轮修复）**：Symbol 计算 accessor（`get [sym]()`/`set [sym]()`）的 getter/setter 不被调用。`get_property_by_symbol`（`js_object.cpp:538`）和 `set_property_by_symbol`（`js_object.cpp:559`）直接读写 `.value` 字段，不检查 `is_accessor` 标志，也不调用 getter/setter 函数。同样，eval_member_expr（`interpreter.cpp:5570-5579`）和 kGetElem VM 路径（`vm.cpp:5420`）在 Symbol key 分支直接调用 `get_property_by_symbol` 而不走 accessor 调用链，导致 `o[sym]` 始终返回 `undefined`，`o[sym] = v` 不触发 setter。
  - **测试结果**：4047/4047 通过，0 LSan 泄漏。

- [x] **计算属性键 `{[expr]: val}` [T262-P2]**（2026-05-26）：
  - **实现内容**：`ObjectProperty` 新增 `key_expr`（`unique_ptr<ExprNode>`）和 `computed`（bool）字段；`ObjectPatternProperty` 新增 `key_expr` 字段；Parser `nud(LBrace)` 四路径（普通/get|set/async/generator`*`）均扩展 `[expr]` computed 键分支；`convert_object_to_pattern` 同步传递 computed/key_expr；`parse_object_binding_pattern` 新增 `[expr]` computed 绑定键支持；`JSObject::define_property_by_symbol` 新方法（懒初始化 symbol_props_，合并 accessor getter+setter）；`SymbolPropertyEntry` 扩展 `getter`/`setter`/`is_accessor` 字段；3 个新 VM opcode（`kSetComputedProp`/`kDefineComputedGetter`/`kDefineComputedSetter`，栈布局 `[obj obj key val/fn]` → pop 3 → push val/fn → kPop）；Interpreter `eval_object_expr` 计算键分支（eval key_expr → Symbol 透传/其他转 string → 各 MethodKind 处理）；Interpreter `bind_pattern(ObjectPattern)` 计算键分支；Compiler `compile_object_expr` 计算键分支 + `hoist_vars_scan_expr` ObjectExpression 分支；Compiler `compile_bind_pattern` 计算键路径（`kGetVar` + `compile_expr(*key_expr)` + `kGetElem`）；VM 3 个 opcode handler（符号键走 set_property_by_symbol/define_property_by_symbol，字符串键走 set_property/define_property，is_method 时设 .name）；`ast_dump.cpp` 扩展计算键输出 `[computed]`；新增 `tests/unit/computed_property_test.cpp`（60 个测试 CP-01～CP-30 × Interp+VM，覆盖基础字符串/数值/null/undefined/NaN/对象/Symbol/WKS 键、计算方法名/.name 属性、getter+setter 合并、`__proto__` 不触发原型设置、计算键解构、嵌套对象、副作用求值顺序、异常传播、多计算键共存）。
  - **测试修复说明**：CP-10 原设计用逗号表达式检测求值顺序，因引擎逗号表达式当前只返回最后项（SequenceExpression 未实现），改为函数调用方式；CP-18 原用 `var a='x', b='y', c='z'` 多声明，因 Parser 解析失败，改为三条独立 var；CP-24 原测试自定义 toString 对象键（需 `to_string_val` 调用 JS 函数），因 `to_string_val` 为 static 方法不支持，改为测试无自定义 toString 的对象键 → `"[object Object]"`。
  - **测试结果**：4011/4011 通过（coverage），0 LSan 泄漏。
