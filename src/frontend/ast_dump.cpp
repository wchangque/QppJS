    ...
    std::visit(overloaded{
                       ...
                       [&](const ObjectExpression& oe) {
                           ...
                           for (const auto& prop : oe.properties) {
                               std::string kind_str;
                               switch (prop.method_kind) {
                                   case MethodKind::kData:        kind_str = "data"; break;
                                   case MethodKind::kMethod:      kind_str = "method"; break;
                                   case MethodKind::kGetter:      kind_str = "getter"; break;
                                   case MethodKind::kSetter:      kind_str = "setter"; break;
...
std::string dump_stmt(const StmtNode& node, int indent) {
    ...
    std::visit(overloaded{
                       ...
                       [&](const ForOfStatement& fo) {
                           std::string decl_info = fo.has_decl
                               ? (fo.var_kind == VarKind::Var     ? "var"
                                  : fo.var_kind == VarKind::Let   ? "let"
                                                                  : "const")
                               : "(no decl)";
                           result = prefix + "ForOfStatement " + decl_info + " " + fo.binding + "\n";
                           result += ind(indent + 1) + "right:\n";
                           result += dump_expr(*fo.right, indent + 2);
                           result += ind(indent + 1) + "body:\n";
                           result += dump_stmt(*fo.body, indent + 2);
                       },
